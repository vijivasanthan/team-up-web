  ...
  function(controllers) {
	...
	//}, 1500);
          ...
          $scope.uploadStepCheckNames = function () {
            ...
            //showNameAlternatives(orderedTeamMembers, '#UploadStepCheckTeamMemberNames');
            $scope.matchedClientsByName = orderMatches($scope.tuSheet.matchedClientForName);
            //showNameAlternatives(orderedClientNames, '#UploadStepCheckClientNames');
          }
          ...
          $scope.uploadStepConfirmNames = function() {
            /*
            var tuSheet = $scope.tuSheet;
            for (var matchedPersonIndex in tuSheet.matchedTeamMemberForName) {
                var matchedPerson = tuSheet.matchedTeamMemberForName[matchedPersonIndex];
                var selectName = "#matchId_" + matchedPerson.matchId;
                var personUuid = $(selectName + " option:selected").val()
                tuSheet.addConfirmedTeamMember(matchedPerson.originalName, personUuid);
            }

            for(var matchedPersonIndex in tuSheet.matchedClientForName) {
              var matchedPerson = tuSheet.matchedClientForName[matchedPersonIndex];
              var selectName = "#matchId_" + matchedPerson.matchId;
              var personUuid = $(selectName + " option:selected").val();
              tuSheet.addConfirmedClient(matchedPerson.originalName, personUuid);
            }
            */
...
*/
	      ...
	      */
          ...
          function TeamUpSpreadsheet(sheet) {
            ...
            this.myTeamUuid = ownTeamUuid();
            this.myTeamClientGroupId = JSON.parse(localStorage['app.teamGroup_' + this.myTeamUuid]).value[0].id;
            ...
            this.uploadTasks = function () {
              ...
              var promises = tasks.map(function(foundTask) {
                  return TeamUp._('taskAdd', null, foundTask.backendTask).then(function(result) {
                    if(result.error) {
            ...
            this.matchClients = function matchClients() {
              var myTeamClients = JSON.parse(localStorage['app.' + this.myTeamClientGroupId]).value;
              angular.forEach(myTeamClients, function(client) {
                client.fullName = '';
                if(client.firstName)
